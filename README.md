...
tests/
  ...
  data_agent_package/                   # 278 CLI + Anthropic/Bedrock/OpenCode client tests
...
All 1420 tests should pass (9 more skip without live LLM credentials) with coverage above 95% (currently ≈97.99%). `uv sync` uses a workspace to build and install both `model-project-constructor` and `model-project-constructor-data-agent` editable in one step.
