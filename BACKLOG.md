...
over-predicts each by under 3.4%. **What a ninth trim must change is FIVE things, none of which the
advice above names on its own:** the `SIZE_PROSE` row keyed to `read_cap` (the shard-banner literal
*"At N lines it sits under the ...-line agent read"*), the `read_cap` key in `SIZES`, the `L12/cap`
arm, the `M63` mutant that is built on that banner string in four places, and the `--self-test`
summary line that prints it — **and the new shard's banner itself, which otherwise re-asserts the
false premise in a file that can never be repaired.** Seven of the eight existing banners carry it.
...
here — `PROJECT_CONVENTIONS.md` §3 mandates the range form `<STEM>-<NEWEST>-through-<OLDEST>.md`
for every non-first shard and seven trims have obeyed it — and `docs/archive/` does not exist in
